...
class Auction {
    ...
    Auction(Player pl, Property pr, List<Player> players) {
        ...
        //Go through all players and add them to auction if they are not the one selling.
        players.forEach(p -> {
            if (p != auctioneer) bidders.add(new Bidder(p));
        });
        turn = bidders.get(0);
        running = true;

        System.out.printf("%nAn auction has begun for %s.%n", property.getFormattedName());
        //Check if anyone already won the auction, otherwise start it.
        if (!winCheck()) {
    ...
    void remove(Player player) {
        System.out.printf("%s exited the auction.%n%n", player.getName());
    ...
    private boolean winCheck() {
        //If there's only 1 bidder left, then they have won.
        if (bidders.size() == 1) {
