...
public class ColorProperty extends Property {
    ...
    void build(Monopoly m) {
        ...
        if (getOwner().getMoney() < getGroup().getBuildPrice()) {
            ...
            houses++;
            System.out.printf("You built a %s on %s for $%d.%n", getHouses() == 5 ? "hotel" : "house", getFormattedName(), getGroup().getBuildPrice());
        }
    }
    ...
    void unbuild(Monopoly m) {
        ...
        System.out.printf("You unbuilt a %s on %s for $%d.%n", getHouses() == 4 ? "hotel" : "house", getFormattedName(), getGroup().getUnbuildPrice());
    }
    ...
    public int rent(List<BoardSpace> allProperties, int roll) {
        ...
        return owned == group.getNumberOfProperties() ? 2 * rents[0] : rents[0];
    }
