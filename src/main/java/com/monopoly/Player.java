...
class Player {
        ...
        for (Property p : getProperties(boardSpaces))
            if (p.isMortgaged()) {
                ...
                if (p instanceof ColorProperty) {
                    ret += ((ColorProperty) p).getHouses() * ((ColorProperty) p).getGroup().getBuildPrice();
                }
            }
    ...
    public Map<String, BiConsumer<Monopoly, String>> makeCommands() {
    ...
    private void CMDRoll(Monopoly m, String s) {
        ...
        if (isInJail()) if (d.isDoubles()) {
            turnInJail = -1;
        } else {
            if (getTurnInJail() >= 3) {
                System.out.println("You must pay.");
                mustPay = true;
            } else System.out.println("You remain in jail.");
            mustRoll = false;
            return;
        }
        ...
        if (space instanceof Property) {
            ...
            Deck deck = cdsp.getType() == CardType.CHANCE ? m.getChance() : m.getCommunityChest();
            Card card = deck.draw();
    ...
    private void CMDForfeit(Monopoly m, String s) {
        for (Property p : getProperties(m.getBoardSpaces())) {
            if (p instanceof ColorProperty) ((ColorProperty) p).resetHouses(m);
            p.setOwner(null);
        }
